...
class BasicBlock(nn.Module):
    expansion: int = 1#BasicBlock和Bottleneck的核心区别之一，输入和输出的通道数是一致的
    ...
    def __init__(
        ...
        self.conv1 = conv3x3(inplanes, planes, stride)#stride=2会降维两倍
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)#不降通道数
        self.bn2 = norm_layer(planes)
        self.downsample = downsample#用一个1*1的卷积核处理，变成想要的通道数，才能相加
        self.stride = stride
...
class Bottleneck(nn.Module):
    ...
    expansion: int = 4#输出通道数是输入通道数的4倍。
...
##############以上新增###########################
    ...
    def _make_layer(
        ...
        )#这个是设置了步长stride的，尺寸小两倍，下面都是默认的stride，不减尺寸
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        #input [128,3,64,64]
        x = self.conv1(x)#output [128,64,32,32]
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)

        x = self.layer1(x)#output [128,64,16,16]#resnet50[128,256,16,16]
        x = self.layer2(x)#output [128,256,8,8]
...
def _resnet(
    ...
    **kwargs: Any,
    ...
    model = ResNet(block, layers, modify_type,**kwargs)
    if pretrained:
...
def resnet18_maxpool(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    return _resnet("resnet18", BasicBlock, [2, 2, 2, 2], pretrained, progress,MODIFY.MAXPOOL,**kwargs)
...
def resnet18_dropout(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet("resnet18", BasicBlock, [2, 2, 2, 2], pretrained, progress,MODIFY.DROPOUT, **kwargs)
